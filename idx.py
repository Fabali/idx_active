...
async def wait_for_workspace_loaded(page, timeout=180):
    ...
    is_workstation_url = (
        ...
        "sherry" in current_url.lower()  # 兼容旧版检测
    )
...
if __name__ == "__main__":
    ...
    separator = "=" * 80
    print(f"\n{separator}")
    log_message(f"开始执行IDX自动登录流程，当前时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{separator}\n")
    ...
    # 添加结束日志
    print(f"\n{separator}")
    log_message(f"IDX自动登录流程执行完成，当前时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{separator}\n")
